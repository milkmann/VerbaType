...
public class DictateInputMethodService extends InputMethodService {
    ...
    private void startWhisperApiRequest() {
        ...
        speechApiThread.execute(() -> {
            try {
                ...
                if (apiHost.equals("custom_server") || apiHost.equals("custom_multimodal")) apiHost = sp.getString("net.devemperor.dictate.transcription_custom_host", getString(R.string.dictate_custom_server_host_hint));
                ...
                switch (transcriptionProvider) {  // for upgrading: use old transcription_model preference
                    ...
                    case 2:
                    case 3:
                        transcriptionModel = sp.getString("net.devemperor.dictate.transcription_custom_model", getString(R.string.dictate_custom_transcription_model_hint));
                        break;
                }

                String resultText;
                if (transcriptionProvider == 3) {
                    String customPrompt = sp.getString("net.devemperor.dictate.transcription_custom_prompt", 
                            "Ты — профессиональный стенографист. Твоя единственная задача — дословно и максимально точно "
                            + "расшифровать присланную аудиозапись в текст. Пиши СТРОГО только расшифрованный текст на языке оригинала (русский или украинский). "
                            + "Не добавляй никаких комментариев от себя, не здоровайся, не объясняй ничего. Если аудио пустое или содержит только шум, верни пустую строку.");
                    resultText = requestMultimodalTranscription(apiHost, apiKey, transcriptionModel, audioFile, customPrompt);
                } else if (transcriptionProvider == 2 && apiHost.toLowerCase().contains("openrouter.ai")) {
                    resultText = requestOpenRouterTranscription(apiHost, apiKey, transcriptionModel, audioFile);
                } else {
                ...
                boolean disableRewording = transcriptionProvider == 3 && sp.getBoolean("net.devemperor.dictate.disable_rewording_for_multimodal", false);
                if (!disableRewording) {
                ...
                if (!disableRewording && !promptsToApply.isEmpty()) {
                    clearQueuedPrompts();
                    if (!livePrompt) {
                ...
                if (cause instanceof InterruptedIOException) {
                    ...
                    mainHandler.post(() -> {
                        ...
                        if (message.contains("api key")) {
                            ...
                            showInfo("internet_error", e.getMessage());
                        }
                    });
                }
            }
    ...
    private String requestOpenRouterTranscription(String apiHost, String apiKey, String transcriptionModel, File audioFile) {
        try {
            ...
            // Build JSON payload
            org.json.JSONObject inputAudio = new org.json.JSONObject();
            inputAudio.put("data", base64Audio);
            inputAudio.put("format", format);

            org.json.JSONObject jsonBody = new org.json.JSONObject();
            jsonBody.put("model", transcriptionModel);
            jsonBody.put("input_audio", inputAudio);
            if (!currentInputLanguageValue.equals("detect")) {
                jsonBody.put("language", currentInputLanguageValue);
            }
            ...
            if (sp.getBoolean("net.devemperor.dictate.proxy_enabled", false)) {
                String proxyHost = sp.getString("net.devemperor.dictate.proxy_host", getString(R.string.dictate_settings_proxy_hint));
                if (!proxyHost.isEmpty() && DictateUtils.isValidProxy(proxyHost)) {
                    java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("^(?:(socks5|http)://)?(?:(\\w+):(\\w+)@)?([\\w.-]+):(\\d+)$");
                    java.util.regex.Matcher matcher = pattern.matcher(proxyHost);
                    if (matcher.matches()) {
                        ...
                        java.net.Proxy.Type proxyType = java.net.Proxy.Type.HTTP;
                        if ("socks5".equalsIgnoreCase(type)) proxyType = java.net.Proxy.Type.SOCKS;
                        java.net.Proxy proxy = new java.net.Proxy(proxyType, new java.net.InetSocketAddress(host, port));
            ...
            while (true) {
                try (okhttp3.Response response = client.newCall(request).execute()) {
                    if (!response.isSuccessful()) {
                        ...
                        String friendlyError = "HTTP error " + response.code();
                        if (response.code() == 401 || response.code() == 403 || lowerMsg.contains("api_key") || lowerMsg.contains("unauthorized")) {
                            friendlyError += ": Invalid API key";
                        } else if (response.code() == 402 || response.code() == 429 || lowerMsg.contains("quota") || lowerMsg.contains("credit") || lowerMsg.contains("balance")) {
                            friendlyError += ": Quota exceeded or rate limited";
                        } else {
                            friendlyError += ": " + errorMsg;
                        }

                        boolean isRetryable = response.code() != 401 && response.code() != 403 && response.code() != 402 && response.code() != 422;
                        if (isRetryable && retryCount < 3) {
                            retryCount++;
                            try { Thread.sleep(3000); } catch (InterruptedException ignored) {}
                            continue;
                    ...
                    return jsonResponse.getString("text").strip();
                } catch (IOException e) {
                    if (retryCount < 3) {
                        retryCount++;
                        try { Thread.sleep(3000); } catch (InterruptedException ignored) {}
    ...
    private String requestMultimodalTranscription(String apiHost, String apiKey, String transcriptionModel, File audioFile, String customPrompt) {
        try {
            // Read audio file bytes
            byte[] fileBytes = java.nio.file.Files.readAllBytes(audioFile.toPath());
            String base64Audio = android.util.Base64.encodeToString(fileBytes, android.util.Base64.NO_WRAP);
            ...
            org.json.JSONObject audioContent = new org.json.JSONObject();
            audioContent.put("type", "input_audio");
            audioContent.put("input_audio", audioData);

            org.json.JSONArray messageContent = new org.json.JSONArray();
            messageContent.put(textContent);
            messageContent.put(audioContent);
            ...
            while (true) {
                try (okhttp3.Response response = client.newCall(request).execute()) {
                    ...
                    org.json.JSONObject messageObjRes = firstChoice.getJSONObject("message");
                    return messageObjRes.getString("content").strip();
                } catch (IOException e) {
                    if (retryCount < 3) {
                        retryCount++;
                        try { Thread.sleep(3000); } catch (InterruptedException ignored) {}
    ...
    private void showInfo(String type, String detailMessage) {
        ...
        if (detailMessage != null && "internet_error".equals(type)) {
            infoTv.setText(detailMessage);
